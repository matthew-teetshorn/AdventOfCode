...
typedef enum { INSERT, LOOKUP, SET, RESET_ONE, RESET_ALL, FREE } regQuery_t;
...
reg_t *regQuery(char *regName, regQuery_t query, reg_t *regContent) {
  ...
  if (query == INSERT) {
    ...
    *regData[regIndex] = *regContent;
    ...
    ep                 = hsearch(e, FIND);
    *(reg_t *)ep->data = *regContent;

  } else if (query == RESET_ONE) {
    ep                             = hsearch(e, FIND);
    ((reg_t *)ep->data)->valueGood = 0;
  } else if (query == RESET_ALL) {
    for (int i = 0; i < regIndex; i++) {
      regData[i]->valueGood = 0;
    }
...
int main() {
  ...
  // Solve for register "a"
  reg_t *eval = regQuery("a", LOOKUP, NULL);
  int final   = evalReg(eval);
  // Reset all registers for next calc
  regQuery(NULL, RESET_ALL, NULL);
  // Change the value in register "b" to what was in "a"
  eval            = regQuery("b", LOOKUP, NULL);
  eval->value     = final;
  eval->valueGood = 1;

  // Recalculate "a" with new "b" value
  eval  = regQuery("a", LOOKUP, NULL);
  final = evalReg(eval);
